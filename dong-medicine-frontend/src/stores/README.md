...
| 属性 | 类型 | 说明 |
|------|------|------|
| `isLoggedIn` | ComputedRef<Boolean> | 是否已登录。判断逻辑：1) token 存在；2) UUID token（无点号）直接返回 true；3) JWT token 解码 payload 检查 exp 未过期 |
...
应用销毁时调用，移除 `auth-expired` 事件监听器。

#### handleAuthExpired()

`auth-expired` 事件回调，调用 `clearAuth()` 清除认证数据。

#### checkTokenExpiry()

检查当前 token 是否过期（使用 buffer 模式），过期则调用 `clearAuth()`。
...
尝试从 localStorage 恢复 token 和用户信息。UUID token（无点号）直接恢复，JWT token 需检查是否过期。

...
- [状态管理] 三种状态获取方式并存：Pinia Store + provide/inject + 直接读取sessionStorage，导致状态来源不一致
- [性能] AdminDashboard.vue和ChartCard.vue全量导入echarts约1MB+，应使用按需导入
- [性能] useMedia.js中computed解构导致响应性丢失
- [代码重复] 三个上传组件(ImageUploader/VideoUploader/DocumentUploader)大量重复逻辑，应抽取useFileUpload composable
- [代码重复] PlantGame.vue和QuizSection.vue大量重复UI结构和逻辑
- [代码重复] 密码校验规则在App.vue和usePersonalCenter.js中重复定义
- [错误处理] useAdminData.js中所有API错误被.catch(() => ({}))静默吞没
- [错误处理] App.vue退出登录失败仍显示成功提示

...
我们的项目不是一个应用，而是8个：MySQL、Redis、RabbitMQ、后端、前端、KKFileView、Prometheus、Grafana。如果用 Docker 一个个启动，你要敲8条命令，还要注意启动顺序（MySQL没启动，后端就连不上数据库）。
...
deploy/
  ...
  |-- setup-ssl.sh        # Let's Encrypt HTTPS 证书申请脚本
  |-- backup-db.sh        # MySQL 数据库备份脚本
  |-- crontab-backup.example # 定时备份 crontab 配置示例
  |-- monitoring/         # Prometheus + Grafana 监控配置
  |   |-- prometheus.yml  #   Prometheus 采集配置
  |   |-- grafana/        #   Grafana 数据源配置
  |-- README.md           # 本文件（你正在看的）
```
...
| `cleanup.sh` | 清理无用 Docker 资源 | 磁盘空间不足时运行 |
| `setup-ssl.sh` | 申请 HTTPS 证书 | 需要启用 HTTPS 时运行 |
| `backup-db.sh` | 备份 MySQL 数据库 | 日常运维，建议配合 crontab 定时执行 |
| `monitoring/` | Prometheus + Grafana 配置 | Docker Compose 自动挂载使用 |
...
我们的 CI/CD 配置在 `.github/workflows/ci-cd.yml`，它分为五个阶段：

```
  代码推送到 main/master 分支
       |
       v
  ======== 阶段一：测试后端 (test-backend) ========
       |
       +-- 检出代码 (checkout)
       +-- 安装 JDK 17
       +-- 运行后端测试 (./mvnw test -B)
  ...
  ======== 阶段二：测试前端 (test-frontend) ========
       ...
       |
       v
  ======== 阶段三：安全扫描 (security-scan) ========
       |
       +-- Trivy 漏洞扫描（仅 main 分支）
       |
       v
  ======== 阶段四：构建和推送 (build-and-push) ========
       |
       +-- Docker Buildx 构建前后端镜像
  ...
  ======== 阶段五：部署 (deploy) ========
       ...
       +-- 启动基础服务 (mysql, redis, rabbitmq, kkfileview)
       +-- 等待基础服务健康检查通过
       +-- 启动后端，等待健康检查通过
       +-- 启动前端
       +-- 清理 SSH 密钥
...
| `RABBITMQ_USER` | RabbitMQ用户名 | 管理员用户名 | `admin` |
| `RABBITMQ_PASSWORD` | RabbitMQ密码 | 自己设一个强密码 | `RabbitMQ!2024` |
...
RABBITMQ_PASSWORD=你的RabbitMQ密码      # RabbitMQ密码
...
# ============ RabbitMQ开关 ============
# 设为 false 可在低内存服务器上禁用 RabbitMQ
APP_RABBITMQ_ENABLED=true

# ============ JVM配置 ============
# 低内存服务器(1.6GB): -Xms256m -Xmx384m -XX:+UseSerialGC
# 4GB+ 服务器: -Xms512m -Xmx768m -XX:+UseG1GC
JAVA_OPTS=-Xms256m -Xmx384m -XX:+UseSerialGC

# ============ AI配置（可选） ============
DEEPSEEK_API_KEY=你的DeepSeek密钥       # 不填则AI聊天功能不可用

# ============ KKFileView配置 ============
KKFILEVIEW_URL=http://kkfileview:8012   # 文档预览服务内部地址

# ============ WebSocket配置 ============
WS_ALLOWED_ORIGINS=http://你的服务器IP   # WebSocket允许的源
```
...
| 资源 | 最低要求 | 推荐配置 | 为什么 |
|------|---------|---------|--------|
| CPU | 2核 | 4核 | 8个Docker容器同时运行，2核勉强够用 |
| 内存 | 4GB | 8GB | MySQL吃1GB+，Redis吃0.5GB，后端吃1GB+，RabbitMQ吃0.5GB，前端+KKFileView+Prometheus+Grafana共约1GB |
| 磁盘 | 40GB | 80GB SSD | Docker镜像+数据+日志，SSD读写快很多 |
| 带宽 | 5Mbps | 10Mbps | 图片和视频资源需要带宽支撑 |

> **省钱提示**：阿里云/腾讯云的学生机（2核4G）可以跑起来（需设置 `APP_RABBITMQ_ENABLED=false` 禁用RabbitMQ），但会比较吃力。如果预算允许，4核8G体验会好很多。
...
- [安全] docker-compose.yml中默认密码过于简单：MYSQL_ROOT_PASSWORD默认root123，REDIS_PASSWORD默认redis123，JWT_SECRET默认your-secret-key-change-in-production，生产环境务必修改
- [安全] RabbitMQ端口(5672/15672)暴露到宿主机，生产环境建议仅在内网通信

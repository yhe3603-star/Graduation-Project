...
项目根目录提供 `docker-compose.yml`，一键启动全部 8 个服务：
...
| RabbitMQ 3.12 | dong-medicine-rabbitmq | 5672/15672 | 消息队列（15672为管理界面） |
...
| Prometheus | dong-medicine-prometheus | 内部 | 监控数据采集 |
| Grafana | dong-medicine-grafana | 3001 | 监控可视化看板 |
...
└── src/
    ...
    │       └── db/
    │           └── migration/       # Flyway 迁移脚本（V1__init_schema.sql, V2__seed_data.sql）
    └── test/                        # 测试代码
        ├── java/com/dongmedicine/
        │   ├── common/              # 工具类单元测试
...
| 层级 | 目录 | 说明 |
|------|------|------|
| 单元测试 | `common/`, `service/impl/`, `controller/` | 工具类、Service、Controller 全覆盖 |
...
| `CORS_ALLOWED_ORIGINS` | CORS 允许的源（逗号分隔） | `http://localhost:3000,https://dongmedicine.com` |
| `WS_ALLOWED_ORIGINS` | WebSocket 允许的源（逗号分隔） | `http://localhost:3000,http://localhost:5173` |
...
- 开发环境默认启用：`spring.flyway.enabled: true`
- 迁移脚本位置：`src/main/resources/db/migration/`
- 生产环境配置 `baseline-on-migrate: true`，适用于已有数据库
...
| **Security Scan** | main 分支（测试通过后） | Trivy 漏洞扫描 |
...
| `setup-ssl.sh` | Let's Encrypt HTTPS 证书申请 |
| `backup-db.sh` | MySQL 数据库每日备份 |
...
| **CORS 白名单** | 从 `app.cors.allowed-origins` 配置读取，不再使用通配符 |
| **验证码增强** | 5 位字母数字混合验证码（排除易混淆字符），忽略大小写验证 |
